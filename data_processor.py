...
class DATA_PREPROCESSOR:
    def __init__(self, ):
        ...
        self.parquet_file_path = r"C:\Users\EDY\Desktop\PROJECT\PI\cloudsail\database\aggregated_database\aggregated_data_60s(test_51turbines_ori).parquet"

        # column mapping, should be depand on original data columns
        self.column_mapping_dict = {
    ...
    def csv2parque(self, ):
        ...
        pq.write_table(table, self.parquet_file_path)
        print(f"Data has been converted and saved to {self.parquet_file_path}")

    def parquet2parquet(self, ):
        # read .parquet and convert to .parquet, make sure that time column covert to PyArrow type
        # this function might not nesessary
        ...
        df_trans.to_parquet(self.parquet_file_path)
        print(f"Data has been converted and saved to {self.parquet_file_path}")

    def rename_columns_in_parquet(self, 
                                  column_mapping_dict=None, 
                                  ):
        # rename columns by inputs columns name mapper dict or self.column_mapping_dict
        
        if column_mapping_dict is not None:
        ...
        try:
            ...
            # change columns name
            df.rename(columns=self.column_mapping_dict, inplace=True)            

            df.to_parquet(self.parquet_file_path)
            print(f"Columns have been renamed and the file has been saved to {self.parquet_file_path}")
    ...
    def label_outliers(
        ...
        outlier_problem_dict['NaN'] = df[df['label_NaN'] != 0]
        
        if outlier_problem_dict['NaN'].empty:
            print("数据无空值。")
        
        """
        给连续时间行存在重复的行打标签
        24.8.6 重复值需要与数据采集确定数据变化频率。
        """
        col_float_set = set(df.columns) & set(col_float)

        condiction_timenotnull = df['label_idtime_null'] == 0
        condiction_timenotduplicated = df['label_time_duplicated'] != 2
